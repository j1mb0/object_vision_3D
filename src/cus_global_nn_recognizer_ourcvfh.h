...
#ifndef CUS_REC_FRAMEWORK_GLOBAL_RECOGNIZER_CVFH_H_
#define CUS_REC_FRAMEWORK_GLOBAL_RECOGNIZER_CVFH_H_
...
template<template<class > class Distance, typename PointInT, typename FeatureT = pcl::VFHSignature308>
  ...
  protected:
    ...
    /** \brief Directory where the models live*/
    ...
    typename boost::shared_ptr<pcl::HypothesisVerification<PointInT, PointInT> > hv_algorithm_;
  ...
  public:
    ...
    bool
    findSourceModel(std::string class_name, PointInTPtr model_cloud_out)
    {
        bool success = false;
        ...
        for (size_t i = 0; i < models->size (); i++)
        {
            ...
            if(cur_model.class_ == class_name)
            {
                success = true;
                model_cloud_out = cur_model.assembled_;
            }
        }
